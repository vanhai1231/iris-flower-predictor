...
const ChatBot = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `${value !== null && value.length > 0 ? `${validate_component(IconButtonWrapper, "IconButtonWrapper").$$render($$result, {}, {}, {
    ...
    })}` : ``} <div class="${escape(null_to_empty(layout === "bubble" ? "bubble-wrap" : "panel-wrap"), true) + " svelte-1nyobg5"}" role="log" aria-label="chatbot conversation" aria-live="polite"${add_attribute("this", div, 0)}>${value !== null && value.length > 0 && groupedMessages !== null ? `<div class="message-wrap svelte-1nyobg5">${each(groupedMessages, (messages, i) => {
    ...
    })} ${pending_message ? `${validate_component(Pending, "Pending").$$render($$result, { layout, avatar_images }, {}, {})}` : `${options ? `<div class="options svelte-1nyobg5">${each(options, (option, index) => {
      return `<button class="option svelte-1nyobg5">${escape(option.label || option.value)} </button>`;
      ...
      {},
      {}
    )}`}</div> ${show_scroll_button ? `<div class="scroll-down-button-container svelte-1nyobg5">${validate_component(IconButton, "IconButton").$$render(
      $$result,
      {
        Icon: ScrollDownArrow,
        label: "Scroll down",
...
//# sourceMappingURL=Index60-zKr3Jzul.js.map
